...
public class OutputHandler {
  ...
  public void showPassOrderSummary(StudyCafePass selectedPass, StudyCafeLockerPass lockerPass) {
    ...
    String discountComment = selectedPass.discountPricePrint();
    if (isDiscountPriceComments(discountComment)) {
      System.out.println(discountComment);
    }

    int totalPrice = selectedPass.getTotalPrice() + getLockerPrice(lockerPass);
    System.out.println("총 결제 금액: " + totalPrice + "원");
    System.out.println();
  }

  private static int getLockerPrice(StudyCafeLockerPass lockerPass) {
    return lockerPass != null ? lockerPass.getPrice() : 0;
  }

  private static boolean isDiscountPriceComments(String discountComment) {
    return discountComment != null;
  }
